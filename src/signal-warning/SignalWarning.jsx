...
const styleBtn = {
  ...
  height: "40px",
  fontWeight: 700,
  backgroundImage: "linear-gradient(90deg, rgba(157, 196, 255, 0.70) 0%, rgba(6, 105, 252, 0.70) 100%)",
  ...
  useEffect(() => {
    ...
    const handleSignalWarning = (receivedData) => {
      ...
      setDataForFilter((prevData) =>
        prevData.map((item) => (item.code === code ? { ...item, ...updatedData } : item))
      );
  
      const result = filterDataBySignalWarnings(updatedData, yourSignalWarningsPopup);
            ...
            <div className="px-48">
              ...
              <div>
                {!loading ? (
                  <>
                    {!alertManagement ? (
                      <SignalBoard data={dataForFilter} yourSignalWarnings={yourSignalWarnings} watchlists={watchlists}/>
                    ) : (
                      <AlertManagement alertManagement={alertManagement} setAlertManagement={setAlertManagement} dataForFilter={dataForFilter} watchlists={watchlists} yourSignalWarnings={yourSignalWarnings} fetchDataSignalWarnings={fetchDataYourSignalWarnings}/>
                    )}
                  </> 
                ) : (
                  <div className="style-loading">
                    <Skeleton.Input active block size="large" className="mt-6" />
                  </div>
                )}
              </div>
            </div>
