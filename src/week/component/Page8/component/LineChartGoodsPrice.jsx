...
const LineChartGoodsPrice = ({ dataChart, name1, name2 }) => {
    ...
    useEffect(() => {
        if (dataChart?.length > 0) {
            const uniqueDates = [...new Set(dataChart?.map(item => moment(item.date).format('DD/MM')))];
            setTimeLine(uniqueDates)
            const result = [];
            dataChart?.forEach(item => {
                const name = item.name === name1 ? `${name1}, cột trái` : (item.name === name2 ? `${name2}, cột phải` : item.name);
