                                    ...
                                    <p className="my-1 text-[11px] font-semibold ">
                                        {data.netVal > 0 ? (
                                            <span>
                                                ...
                                                <span className="text-green-600 text-[13px] font-bold">mua</span>{" "}
                                                ròng <span className="text-green-600 text-[13px]">{formatNumber(data.netVal / 1000000000)}</span>{" "}
                                                tỷ đồng trên sàn HOSE, tiêu điểm là{" "}
                                                {data.topBuy.map((item, index) => (
                                                    <span
                                                        key={item.code}
                                                    >
                                                        <span className="text-green-600 text-[12px]">{item.code}</span>  <span>({formatNumber(item.point / 1000000000)} tỷ đồng)</span>
                                                        {index !== data.topBuy.length - 1 ? ", " : ""}
                                                    </span>
                                                ))}. Ở chiều ngược
                                                lại, khối ngoại <span className="text-red-600 text-[13px] font-bold">bán</span> ròng mạnh tại các cổ phiếu {data.topSell.map((item, index) => (
                                                    <span
                                                        key={item.code}
                                                    >
                                                        <span className="text-red-600 text-[12px]">{item.code}</span>  <span>({formatNumber(item.point / 1000000000)} tỷ đồng)</span>
                                            ...
                                            <>
                                                • Trong tuần giao dịch ròng của khối ngoại:{" "}
                                                <span className="text-red-600 text-[13px] font-bold">bán</span>{" "}
                                                ròng <span className="text-red-600 text-[13px]">{formatNumber(data.netVal / 1000000000)}</span>{" "}
                                                tỷ đồng trên sàn HOSE, tiêu điểm là{" "}
                                                ...
                                                lại, khối ngoại <span className="text-green-600 text-[13px] font-bold">mua</span> ròng mạnh tại các cổ phiếu {data.topBuy.map((item, index) => (
                                                    <span
                                                        key={item.code}
                                                    >
                                                        <span className="text-green-600 text-[12px]">{item.code}</span>  <span>({formatNumber(item.point / 1000000000)} tỷ đồng)</span>
