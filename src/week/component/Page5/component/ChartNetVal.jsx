...
const ChartNetVal = ({ data }) => {
  ...
  const dataProprietary = data?.map((item) => +item.net_proprietary.toFixed(2));
  const dataRetail = data?.map((item) => +item.net_retail.toFixed(2));
  const timeLine = data?.map((item) => moment(item.date).format("DD/MM"));
