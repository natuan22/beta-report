...
const Table = ({ data }) => {
    return (
        <div className='relative'>
            <div className='absolute w-[780px] h-[3px] bg-[#F89637] bottom-[55px] left-0 z-10'>

            </div>
            <table className='w-[780px] h-[260px] border border-1 border-solid border-[#143A65] translate-y-[-30px] border-collapse'>
                    ...
                    {data?.map((item, index) => {
                        return (
                            <tr className='text-center font-semibold' key={index}>
                                ...
                                {item.name !== 'SAR' ? (
                                    <>
                                        <td className={`${getColorBaseOnName(item.single)} p-1`}>
                                            {item.single}
                                        </td>
                                        <td className={`${getColorBaseOnName(item.hat)} p-1`}>
                                            {item.hat}
                                        </td>
                                    </>
                                ) : (
                                    <td colSpan={2} className={`${getColorBaseOnName(item.single)} p-1`}>
                                        {item.single}
                                    </td>
                                )}
                            </tr>
