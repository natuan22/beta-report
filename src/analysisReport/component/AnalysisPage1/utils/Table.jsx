                        ...
                        data.map((item, index) => {
                            return (
                                <tr key={index} className='text-center text-[13px] font-semibold'>
                                    <td>{item.code}</td>
                                    <td className={`${getColorBaseOnValue(item.month)} `}>{formatNumberMorning(item.month)}%</td>
                                    <td className={`${getColorBaseOnValue(item.month_3)} `}>{formatNumberMorning(item.month_3)}%</td>
                                    <td className={`${getColorBaseOnValue(item.ytd)} `}>{formatNumberMorning(item.ytd)}%</td>
                                </tr>
                            )
                        })
                        : <div>Loading...</div>}
