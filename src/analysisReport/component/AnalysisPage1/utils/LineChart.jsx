...
const LineChart = ({ data }) => {
    ...
    const uniqueDates = Array.from(new Set(data.map(item => moment(item.date).format('MM/DD'))));
    ...
    data.forEach(item => {
        ...
        seriesData[item.code].push([moment(item.date).format('MM/DD'), item.value]);
    });
    ...
    const options = {
        ...
        legend: {
            ...
            labelFormatter: function () {
                return '<span style="font-size: 12px; font-weight: bold; color: #333333">' + this.name + '</span>';
            }
        },
        xAxis: [{
