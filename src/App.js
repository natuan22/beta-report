...
function App() {
  ...
  const RouteWithTitle = ({ path, component: Component, title, role, requiresLogin }) => {
    ...
    return <PrivateRoute component={Component} role={role} requiresLogin={requiresLogin} />;
  };

  // Mapping routes and applying RouteWithTitle for each route
  const mappedRoute = routes.map(({ path, component: Component, role, requiresLogin, title }) => (
    <Route 
      key={path} 
      path={path} 
