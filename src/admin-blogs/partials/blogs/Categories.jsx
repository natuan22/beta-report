  ...
  const renderCategoryRow = (category, index, depth = 0) => {
    ...
    return (
      <Fragment key={index}>
        <tr>
          ...
          <td className="p-2 whitespace-nowrap">
            <div>
              <div style={{ paddingLeft: `${depth * 40}px` }}>
                <div className="font-medium text-gray-800 dark:text-gray-100 text-lg hover:underline cursor-pointer" onClick={(e) => { e.stopPropagation(); showModalEit(category) }}>
                  {category.name}
                </div>
          ...
          <td className="p-2 whitespace-nowrap">
            <div className="text-center">
              {category.published === 1 ? <IoMdCheckmarkCircleOutline className="w-6 h-6 text-green-400"/>: <IoMdCloseCircleOutline className="w-6 h-6 text-red-400"/>}
            </div>
          </td>
          <td className="p-2 whitespace-nowrap">
            <div className="text-center">
        ...
        {isExpanded && category.children && category.children.map((childCategory, childIndex) => renderCategoryRow(childCategory, childIndex, depth + 1))}
      </Fragment>
    );
  };
            ...
            <table className="table-auto w-full">
              ...
              <tbody className="text-sm divide-y divide-gray-100 dark:divide-gray-700/60">
                {!loading && dataCategories ? (
                  dataCategories.map((category, index) => renderCategoryRow(category, index))
                ) : (
                  <tr>
                    <td colSpan={5}>
                      <div className="my-14">
