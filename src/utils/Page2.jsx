...
const Page2 = () => {
    ...
    useEffect(() => {
        ...
        const fetchDataStockMarket = async () => {
            try {
                const response = await https.get('api/v1/report/thi-truong-chung-khoan')
    ...
    return (
        <div className="h-[1480px] w-[800px]  ">
            ...
            <div className="content-top w-[780px] h-[310px] z-10 relative  flex justify-evenly ml-2 mt-2 ">
                <div>
                    <h2 className='text-[16px] font-bold text-[#0155B7] text-center my-1'>Tỷ giá</h2>
                            ...
                            {rate?.map(item => {
                                return (
                                    <tr>
                                        ...
                                        <td className='text-center px-2 py-1'>{formatNumberMorning(item.price)}</td>
                                        <td className={`${getColorBaseOnValue(item.day)} text-center px-1 py-1`}>{formatNumberMorning(item.day)}</td>
                                        <td className={`${getColorBaseOnValue(item.month)} text-center px-1 py-1`}>{formatNumberMorning(item.month)}</td>
                                        <td className={`${getColorBaseOnValue(item.year)} text-center px-1 py-1`}>{formatNumberMorning(item.year)}</td>
                                    </tr>
                                )
                            }
                            )}
                ...
                <div>
                    <h2 className='text-[16px] font-bold text-[#0155B7] text-center my-1'>Lãi suất bình quân liên ngân hàng</h2>
                    <table className='bg-transparent border-collapse w-[375px]  h-[90%]'>
                        <thead className='bg-[#0155B7]  border-1 border-[#0155B7] border-solid border-collapse'>
                            <tr className='text-white '>
                        ...
                        <tbody className="border-1 border-[#0155B7] border-solid border-collapse  ">
                            {interestRate?.map(item => (
                                <tr>
                                    ...
                                    <td className='text-center px-2 py-3'>{formatNumberMorning(item.price)}</td>
                                    <td className={`${getColorBaseOnValue(item.day)} text-center px-1 py-3`}>{formatNumberMorning(item.day)}</td>
                                    <td className={`${getColorBaseOnValue(item.month)} text-center px-1 py-3`}>{formatNumberMorning(item.month)}</td>
                                    <td className={`${getColorBaseOnValue(item.year)} text-center px-1 py-3`}>{formatNumberMorning(item.year)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
            ...
            <div className="content-mid w-[790px] h-[300px] z-10 relative ml-2  ">
                <div className='w-[760px] translate-x-[10px]'>
                    <h2 className='text-[16px] font-bold text-[#0155B7] text-center m-1'>Giá hàng hóa</h2>
                    <table className='bg-transparent border-collapse w-[100%]'>
                        <thead className='bg-[#0155B7]  border-1 border-[#0155B7] border-solid border-collapse'>
                            <tr className='text-white '>
                                <th className='font-semibold px-2 py-1 text-left w-[182px] '>Hàng hóa</th>
                                <th className='font-semibold px-1 py-1 '>Thị giá</th>
                                <th className='font-semibold px-1 py-1 '>%D</th>
                                <th className='font-semibold px-1 py-1 '>%1M</th>
                                <th className='font-semibold px-1 py-1 '>%YtD</th>
                        ...
                        <tbody className="border-1 border-[#0155B7] border-solid border-collapse ">
                            {goodsPrice?.slice(0, 8)?.map(item => (
                                <tr>
                                    ...
                                    <td className={`${getColorBaseOnValue(item.day)} text-center px-1 py-1`}>{formatNumberMorning(item.day)}</td>
                                    <td className={`${getColorBaseOnValue(item.month)} text-center px-1 py-1`}>{formatNumberMorning(item.month)}</td>
                                    <td className={`${getColorBaseOnValue(item.ytd)} text-center px-1 py-1`}>{formatNumberMorning(item.ytd)}</td>
                                    <td className={`${getColorBaseOnValue(item.year)} text-center px-1 py-1`}>{formatNumberMorning(item.year)}</td>
                                </tr>
                            ))}
                        </tbody>
            ...
            <div className="content-bot w-[790px] h-[300px] z-10 relative ml-2  ">
                <div className='w-[760px] translate-x-[10px]'>
                    ...
                    <table className='bg-transparent border-collapse w-[100%]'>
                        <thead className='bg-[#0155B7]  border-1 border-[#0155B7] border-solid border-collapse'>
                            <tr className='text-white '>
                                <th className='font-semibold px-2 py-1 text-left w-[120px] '>Chỉ số</th>
